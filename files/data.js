...
function getDistrictKey(input) {
  ...
  for (const [key, d] of Object.entries(DISTRICTS)) {
    ...
    if (d.aliases && d.aliases.some(a => a.toLowerCase() === clean)) return key;
    const nameMatch = [d.nameEN, d.nameHI, d.nameMR].some(n => n && n.toLowerCase() === clean);
    if (nameMatch) return key;
  }
  // partial match
  for (const [key, d] of Object.entries(DISTRICTS)) {
    if (key.includes(clean) || clean.includes(key)) return key;
    if (d.nameEN && d.nameEN.toLowerCase().includes(clean)) return key;
    if (d.aliases && d.aliases.some(a => a.toLowerCase().includes(clean))) return key;
  }
  return null;
}
...
const LANG = {
  en: {
    ...
    lblDistrict:"District", lblSize:"Land Size (Acres)", lblSoil:"Soil Type",
    lblSeason:"Season", lblIrr:"Irrigation", lblConcern:"Special Concern (optional)",
  ...
  hi: {
    ...
    lblDistrict:"जिला", lblSize:"भूमि क्षेत्र (एकड़)", lblSoil:"मिट्टी का प्रकार",
    lblSeason:"मौसम", lblIrr:"सिंचाई", lblConcern:"विशेष समस्या (वैकल्पिक)",
    btnRec:"सिफारिश पाएं",
  ...
  mr: {
    ...
    lblDistrict:"जिल्हा", lblSize:"जमीन क्षेत्र (एकर)", lblSoil:"मातीचा प्रकार",
    lblSeason:"हंगाम", lblIrr:"सिंचनाचा प्रकार", lblConcern:"विशेष समस्या (पर्यायी)",
    btnRec:"शिफारस मिळवा",
