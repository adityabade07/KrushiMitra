...
function showDistrictBadge(key) {
  ...
  const kharif = d.crops.kharif.slice(0,3).map(c=>c.name).join(', ');
  grid.innerHTML = `
    ...
    <div class="db-item">${cropsLabel}: ${kharif}</div>
  `;
...
function updateHomeCard(key) {
  const d = DISTRICTS[key];
  if (!d) return;
  const crops = d.crops[currentSeason] || d.crops.kharif;
  const crop = crops[0];
  const dname = getDistrictName(key, currentLang);
  setTxt('home-crop-name', crop.name);
  setTxt('home-crop-detail', getSeasonLabel(currentSeason) + ' — ' + dname);
  setTxt('home-yield', crop.yield.split('-')[0] + ' Q');
  const riskItems = [
    {id:'home-risk1',type:'drought',val:d.droughtRisk},
    {id:'home-risk2',type:'pest',val:d.pestRisk},
    {id:'home-risk3',type:'flood',val:d.floodRisk}
  ];
  riskItems.forEach(r => {
...
function getRecommendation() {
  ...
  currentSize = parseFloat(document.getElementById('f-size').value) || 2;
...
function renderRecommendation(d) {
  ...
  setTxt('rec-sub', dname + ' — ' + getSeasonLabel(currentSeason) + ' — ' + d.soilTypes[0]);
  setTxt('rec-yield', crop.yield);
...
function selectCrop(el, idx) {
  ...
  setTxt('analysis-text', c.notes + ' ' + d.notes);
  renderCalendarForCrop(c);
}

function buildAnalysis(d, crop, dname) {
  const s = currentSeason;
  if (currentLang==='hi')
    return `${dname} जिले की ${d.soilTypes[0]} मिट्टी और ${d.rainfall}mm वार्षिक वर्षा के लिए ${getSeasonLabel(s)} मौसम में ${crop.name} सर्वोत्तम है। बुवाई: ${crop.sow}। उपज: ${crop.yield} प्रति एकड़। MSP ₹${crop.msp}/क्विंटल। ${crop.notes}`;
  if (currentLang==='mr')
    return `${dname} जिल्ह्याच्या ${d.soilTypes[0]} माती आणि ${d.rainfall}mm पावसासाठी ${getSeasonLabel(s)} हंगामात ${crop.name} उत्तम आहे। पेरणी: ${crop.sow}। उत्पन्न: ${crop.yield} प्रति एकर। MSP ₹${crop.msp}/क्विंटल. ${crop.notes}`;
  return `${crop.name} is recommended for ${dname}'s ${d.soilTypes[0]} and ${d.rainfall}mm annual rainfall during ${getSeasonLabel(s)} season. Sow: ${crop.sow}. Yield: ${crop.yield}/acre. MSP: ₹${crop.msp}/quintal. ${crop.notes}`;
}
...
function renderRisk(d) {
  ...
  setRiskCard('drought', d.droughtRisk);
  setRiskCard('flood', d.floodRisk);
  setRiskCard('pest', d.pestRisk);
  const sug = document.getElementById('sug-list');
  if (sug) sug.innerHTML = getRiskSuggestions(d, dname).map(s => `<div class="sug-item"><p>${s}</p></div>`).join('');
}
...
function getRiskSuggestions(d, dname) {
  ...
  if (d.pestRisk==='high') {
    sugs.push(hi ? `${dname} में कीट का जोखिम अधिक है। मुख्य कीट: ${d.specialPests.join(', ')}। फेरोमोन ट्रैप लगाएं।` :
      mr ? `${dname}मध्ये कीटकांचा धोका जास्त आहे. मुख्य कीटक: ${d.specialPests.join(', ')}. फेरोमोन ट्रॅप वापरा.` :
      `${dname} has high pest risk. Key pests: ${d.specialPests.join(', ')}. Install pheromone traps.`);
...
function renderCalendar(d) {
  ...
  setTxt('cal-loc-info', dname + ', Maharashtra — ' + currentSize + ' Acres');
...
function renderDashboard(d) {
  ...
  setTxt('dash-loc', dname + ', Maharashtra — ' + currentSize + ' Acres — ' + soilLabel + ' — ' + seasonLabel);
  ...
  setTxt('dk-yield', t.yldLabel); setTxt('dv-yield', crop.yield);
  ...
  setTxt('dk-drought', t.droughtLbl); setTxt('dv-drought', t[d.droughtRisk] || d.droughtRisk);
  setTxt('dk-flood', t.floodLbl); setTxt('dv-flood', t[d.floodRisk] || d.floodRisk);
  setTxt('dk-pest', t.pestLbl); setTxt('dv-pest', t[d.pestRisk] || d.pestRisk);
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.radio-option input').forEach(inp => {
    inp.addEventListener('change', function() {
      ...
      if (this.name==='season') { currentSeason = this.value; if (appReady) renderAll(DISTRICTS[currentDistKey]); }
      if (this.name==='soil') currentSoil = this.value;
    });
  });
